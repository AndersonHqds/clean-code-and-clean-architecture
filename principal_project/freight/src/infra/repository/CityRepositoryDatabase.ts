...
export default class CityRepositoryDatabase implements CityRepository {
  ...
  async getByZipcode(code: string): Promise<City> {
    const [cityData] = await this.connection.query(
      "select id_city, name, lat, long from ccca_freight.zipcode join ccca_freight.city using (id_city) where code = $1",
      [code]
    );
